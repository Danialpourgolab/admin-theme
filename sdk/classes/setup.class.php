...
if ( ! class_exists( 'ADMTH_Setup' ) ) {
  class ADMTH_Setup {
    ...
    public function __construct() {
      ...
      // Init action
      do_action( 'admth_init' );

      // Setup textdomain
      self::textdomain();

      add_action( 'after_setup_theme', array( 'ADMTH', 'setup' ) );
      add_action( 'init', array( 'ADMTH', 'setup' ) );
      add_action( 'switch_theme', array( 'ADMTH', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'ADMTH', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'ADMTH', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'ADMTH', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'ADMTH', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'ADMTH_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( ADMTH_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'ADMTH_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            ADMTH_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'ADMTH_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              ADMTH_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'admth_override', 'admth-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'admth_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'ADMTH_Field_'. $field ) && class_exists( 'ADMTH_Fields' ) ) {
    ...
    public static function textdomain() {
      load_textdomain( 'admth', self::$dir .'/languages/'. get_locale() .'.mo' );
    }
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! self::$enqueue ) {
        ...
        if ( $wpscreen->id === 'tools_page_admth-welcome' ) {
          self::$enqueue = true;
        }

      }

      if ( ! apply_filters( 'admth_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      // Main style
      wp_enqueue_style( 'admth', self::include_plugin_url( 'assets/css/style'. $min .'.css' ), array(), self::$version, 'all' );

      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'admth-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }

      // Main scripts
      wp_enqueue_script( 'admth-plugins', self::include_plugin_url( 'assets/js/plugins'. $min .'.js' ), array(), self::$version, true );
      wp_enqueue_script( 'admth', self::include_plugin_url( 'assets/js/main'. $min .'.js' ), array( 'admth-plugins' ), self::$version, true );

      // Main variables
      wp_localize_script( 'admth', 'admth_vars', array(
        'color_palette'     => apply_filters( 'admth_color_palette', array() ),
        'i18n'              => array(
          'confirm'         => esc_html__( 'Are you sure?', 'admth' ),
          'typing_text'     => esc_html__( 'Please enter %s or more characters', 'admth' ),
          'searching_text'  => esc_html__( 'Searching...', 'admth' ),
          'no_results_text' => esc_html__( 'No results found.', 'admth' ),
        ),
      ) );
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'ADMTH_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'admth-google-web-fonts', esc_url( '//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );

          wp_localize_script( 'admth-google-web-fonts', 'WebFontConfig', array( 'google' => array( 'families' => $fonts ) ) );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field['_notice'] ) ) {
        ...
        $field            = array();
        $field['content'] = esc_html__( 'Oops! Not allowed.', 'admth' ) .' <strong>('. $field_type .')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' admth-pseudo-field' : '';
      ...
      if ( ! empty( $field['dependency'] ) ) {
        ...
        $visible = ( ! empty( $depend_visible ) ) ? ' admth-depend-visible' : ' admth-depend-hidden';

      }

      // These attributes has been sanitized above.
      echo '<div class="admth-field admth-field-'. $field_type . $is_pseudo . $class . $visible .'"'. $depend .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="admth-title">';
          echo '<h4>'. $field['title'] .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="admth-subtitle-text">'. $field['subtitle'] .'</div>' : '';
          echo '</div>';
        }

        echo ( ! empty( $field['title'] ) ) ? '<div class="admth-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'ADMTH_Field_'. $field_type;

        if ( class_exists( $classname ) ) {
          $instance = new $classname( $field, $value, $unique, $where, $parent );
          $instance->render();
        } else {
          echo '<p>'. esc_html__( 'Field not found!', 'admth' ) .'</p>';
        }
...
ADMTH_Setup::init( __FILE__, false );
...
if ( ! class_exists( 'ADMTH' ) ) {
  class ADMTH extends ADMTH_Setup{}
}
