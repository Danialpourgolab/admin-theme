 ...
 *
 */
if ( ! class_exists( 'ADMTH_Field_link_color' ) ) {
  class ADMTH_Field_link_color extends ADMTH_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'admth' ),
        'hover'    => esc_html__( 'Hover', 'admth' ),
        'active'   => esc_html__( 'Active', 'admth' ),
        'visited'  => esc_html__( 'Visited', 'admth' ),
        'focus'    => esc_html__( 'Focus', 'admth' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="admth--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="admth-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
